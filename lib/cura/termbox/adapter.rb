    ...
    class Adapter < Cura::Adapter
        ...
        event = case event[:type]
          when ::Termbox::TB_EVENT_KEY
            if event[:key] != 0
              ...
              character = [event[:ch]].pack("U") # UTF-8 character from unicode
      ...
      def convert_termbox_control_key_to_cura_event(key)
        case key
          ...
          when Termbox::Termbox::TB_KEY_CTRL_LSQ_BRACKET then Event::KeyDown.new(name: :left_bracket, control: true) # clash with 'ESC'
